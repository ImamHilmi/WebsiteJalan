	...
	function discy_meta($date = "",$category = "",$comment = "",$asked = "",$icons = "",$views = "",$post_id = 0) {
		...
		if ( $date == "on" ) {
			...
			if (is_single() && 'question' === $post_type) {
				$puplished_date = (get_the_time('c',($post_id)))?get_the_time('c',($post_id)):get_the_modified_date('c',($post_id));
				echo '<span class="discy_hide" itemprop="dateCreated" datetime="'.$puplished_date.'"></span>
				<span class="discy_hide" itemprop="datePublished" datetime="'.$puplished_date.'"></span>';
			}
			echo '</span>';
		}
